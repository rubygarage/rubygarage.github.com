...
## Code smell
Code smell is any symptom in the source code of a program that possibly indicates a deeper problem. Code smells are
usually not bugs they are not technically incorrect and don't currently prevent the program from functioning. Instead,
they indicate weaknesses in design that may be slowing down development or increasing the risk of bugs or failures in
the future.
...
* Communication: The reader must decide whether two things are really expressing one concept, and whether any
differences are significant.
### What to Do
* If in same class [Extract Method](#extract-method-before)
* If in two subclasses [Extract Method](#extract-method-before) or [Pull Up Method](#pull-up-method-before)
* If not identical [Form Template Method](#form-template-method-before) or [Subtitute
Algorithm](#substitute-algorithm-before)
* If in unrelated locns [Extract Class](#extract-class-before) or [Extract Module](#extract-module-before)
...
### What to Do
* If no prob with params use [Extract Method](#extract-method-before) to break up the method into smaller pieces. Look
...
* If the method doesn't separate easily into pieces, consider [Replace Method with Method
Object](#replace-method-with-method-object-before) to turn the method into a separate object, or [Replace Temp with
Query](#replace-temp-with-query-before)
* If has loops and conditionals [Decompose Conditional](#decompose-conditional-before) or Replace Loop with [Collection
Closure Method](#collection-closure-method-before)
...
### What to Do
* If a parameter's value can be obtained from another object use [Replace Parameter with
Method](#replace-parameter-with-method-before).
* If the parameters come from a single object, try [Preserve Whole Object](#preserve-whole-object-before).
* If the data is not from one logical object, you still might group them via [Introduce Parameter
Object](#introduce-parameter-object-before).
* Otherwise make clearer with [Introduce Named Parameter](#introduce-named-parameter-before)
...
### What to Do
* Use [Rename Method](#rename-method-before) (or field, constant, etc.) to give it a better name
...
### What to Do
* When a comment explains a code fragment, you can often use [Extract Method](#extract-method-before) to pull the
...
Method](#rename-method-before) using the comment as the basis of the new name.
* When a comment explains preconditions, consider using [Introduce Assertion](#introduce-assertion-before) to replace
...
Greedy Module.
### What to Do
* To break up the module further, use [Extract Class](#extract-class-before) or [Extract
Module](#extract-module-before) if you can identify a new piece that has part of this module's responsibilities
* Also will be useful [Replace Type Code with Polymorphism](#replace-type-code-with-polymorphism-before), [Replace Type
Code with Module Extension](#replace-type-code-with-module-extension-before), [Replace Type Code with State
Strategy](#replace-type-code-with-state-strategy-before)
...
use [Extract Module](#extract-module-before) to create a new one.
* Use [Move Field](#move-field-before) and Move Method to put the functionality onto the chosen module. After the
...
### What to Do
* If the envious code fragment is not isolated, use [Extract Method](#extract-method-before) to pull it into its own
method.
* Look for the class of the object that is referenced most and use [Move Field](#move-field-before) and Move Method to
...
### What to Do
* If the items are instance variables in a class, use [Extract Class](#extract-class-before) to pull them into a new
class.
* If the values are together in method signatures, use [Introduce Parameter
Object](#introduce-parameter-object-before), [Replace Array with Object](#replace-array-with-object-before) to
...
Polymorphism](#replace-type-code-with-polymorphism-before) or [Replace Type Code with Module
Extension](#replace-type-code-with-module-extension-before) or [Replace Type Code with State
Strategy](#replace-type-code-with-state-strategy-before). You may find things become more clear if you first use
Extract Method on the clauses.
* If used once [Replace Parameter with Method](#replace-parameter-with-method-before)
* If one branch is null [Introduce Null Object](#introduce-null-object-before)
* If have dificult conditions use [Decompose conditional](#decompose-conditional-before), [Substitute
Algorithm](#substitute-algorithm-before), [Replace Method With Method
Object](#replace-method-with-method-object-before)
...
paths.
### What to Do
* Use [Extract Method](#extract-method-before) to strip the controlled method down to the bare skeleton.
* Then use [Inline Method](#inline-method-before) to push the responsibility back up to the caller(s).
...
[Extract Method](#extract-method-before) to hide detail behind an intention revealing name.
...
### What to Do
* If the module has too many (i.e., more than one) responsibilities use [Extract Class](#extract-class-before) or
[Extract Module](#extract-module-before) to separate the responsibilities.
* If several classes share the same decisions or variation points, you may be able to consolidate them into new classes
(e.g., by [Hide Delegate](#hide-delegate-before), [Pull Up Method](#pull-up-method-before), [Form Template
Method](#form-template-method), Extract Superclass or Extract Subclass) or extract a common module to serve as a
...
*Collapse Hierarchy*.
* Otherwise, fold its behavior into its caller via [Inline Class](#inline-class-before).
...
### What to Do
* Use [Extract Class](#extract-class-before), moving over the fields and any related code.
* Also [Introduce Null Object](#introduce-null-object-before), [Replace Method with Method
Object](#replace-method-with-method-object-before), [Replace Temp with Query](#replace-temp-with-query-before)
...
### What to Do
* If the manipulations actually belong on the target object (the one at the end of the chain), use [Extract
Method](#extract-method-before) and [Move Method / Move Feild](#move-field-before) to put them there.
* Use [Hide Delegate](#hide-delegate-before) and [Inline Class](#inline-class-before) to make the caller depend only on
...
def self.find id
# database operation to retrieve data. We will simulate it for now.
...
(room_range.low >= @range.low) && (room_range.high
